...
function build_model(;
    ...
    H,
    X,
    rho = JuMP.Containers.DenseAxisArray(fill(0, length(1:S)), 1:S),
    beta = JuMP.Containers.DenseAxisArray(fill(0, length(1:S)), 1:S),
    alpha = JuMP.Containers.DenseAxisArray(fill(0, length(1:W)), 1:W),
    E = JuMP.Containers.DenseAxisArray(
        fill(0, length(1:S), length(1:W), length(1:H)),
        1:S,
        1:W,
        1:H,
    ),
    C = JuMP.Containers.DenseAxisArray(
        fill(0, length(1:S), length(1:W)),
        1:S,
        1:W,
    ),
    ...
    polyX = JuMP.Containers.DenseAxisArray(
        fill(0, length(1:X), length(1:(3+H))),
        1:X,
        1:(3+H),
    ),
)
